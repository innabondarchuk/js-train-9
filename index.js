...
function getPositiveArrayValues(arr) {
  ...
  const positiveArrayValues = Array.from(arr.values());

  return positiveArrayValues.filter(element => element > 0);
}
...
function removeAndDouble(arr, startIdx) {
  ...
  const removeAnd = arr.splice(startIdx);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const squaredArray = flattenArray.map(element => element ** 2);
...
function capitalizeWords(arr) {
  ...
  const capitalizedArray = arr.map((word) => word.charAt().toUpperCase() + word.slice(1));
...
function calculateTotalPrice(arr) {
  ...
  const sum = arr.reduce((accumulator, currentValue) => accumulator + currentValue.price, 0);
...
function findLastNegativeNumberIndex(arr) {
  ...
  const lastNegativeNumbe = arr.findLastIndex(element => element < 0);

  return lastNegativeNumbe;
}
